...
class form1 extends StatelessWidget {
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      title: appTitle,
      home: Scaffold(
        appBar: AppBar(title: Text(appTitle), backgroundColor: const Color.fromARGB(143, 0, 0, 255)),
        body: MyCustomForm1(),
      ),
    );
  }
                    ...
                    data: Theme.of(context).copyWith(
                      ...
                      textButtonTheme: TextButtonThemeData(
                        style: TextButton.styleFrom(
                          primary: const Color.fromARGB(143, 0, 0, 255), // button text color
                        ),
                      ),
                    ),
                    child: child!,
...
Future createUser(User user) async {
  ...
  final docUser = db.collection('userdata').doc(uid).collection('debit');
  user.id = docUser.id;
